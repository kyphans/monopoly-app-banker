...
export const MainGame: React.FC = () => {
  ...
  const [transferType, setTransferType] = useState<'bank-to-any' | 'any-to-bank' | 'p1-to-p2' | 'p2-to-p1' | 'p1-to-bank' | 'p2-to-bank' | 'bank-to-p1' | 'bank-to-p2'>('bank-to-p1');
  ...
  const handleDragStart = (source: 'p1' | 'p2' | 'bank', e: MouseEvent | TouchEvent | PointerEvent) => {
  ...
  const handleDragEnd = () => {
    ...
    if (source && target && source !== target) {
      ...
      let type: 'bank-to-any' | 'any-to-bank' | 'p1-to-p2' | 'p2-to-p1' | 'p1-to-bank' | 'p2-to-bank' | 'bank-to-p1' | 'bank-to-p2' = 'bank-to-p1';
    ...
    <div className="flex-1 flex flex-col h-full bg-background-dark relative select-none">
      ...
      >
        <div className="space-y-6">
          <div className="space-y-3">
            <div className="flex items-center justify-between p-5 bg-slate-50 dark:bg-slate-800/50 rounded-4xl border border-slate-100 dark:border-slate-800 shadow-inner">
              ...
              <div className="flex flex-col items-center justify-center flex-1">
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">From</span>
                <div className="flex flex-col items-center space-y-2">
                  <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-white font-black shadow-lg ${
                    transferType.startsWith('p1') ? 'bg-blue-500 shadow-blue-500/20' : 
                    transferType.startsWith('p2') ? 'bg-rose-500 shadow-rose-500/20' : 
                    'bg-amber-600 shadow-amber-600/20'
                  }`}>
                    {transferType.startsWith('p1') ? players[0].name.charAt(0) : 
                     transferType.startsWith('p2') ? players[1].name.charAt(0) : 
                     <Landmark size={24} />}
                  </div>
                  <div className="text-center overflow-hidden w-full px-1">
                    <p className={`text-[10px] font-black uppercase truncate tracking-tight ${
                      transferType.startsWith('p1') ? 'text-blue-500' : 
                      transferType.startsWith('p2') ? 'text-rose-500' : 
                      'text-amber-600 dark:text-amber-500'
                    }`}>
                      {transferType.startsWith('p1') ? players[0].name : 
                       transferType.startsWith('p2') ? players[1].name : 
                       'The Bank'}
                    </p>
                  </div>
                </div>
              ...
              {/* To Participant */}
              <div className="flex flex-col items-center justify-center flex-1">
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">To</span>
                <div className="flex flex-col items-center space-y-2">
                  <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-white font-black shadow-lg ${
                    transferType.endsWith('p1') ? 'bg-blue-500 shadow-blue-500/20' : 
                    transferType.endsWith('p2') ? 'bg-rose-500 shadow-rose-500/20' : 
                    'bg-amber-600 shadow-amber-600/20'
                  }`}>
                    {transferType.endsWith('p1') ? players[0].name.charAt(0) : 
                     transferType.endsWith('p2') ? players[1].name.charAt(0) : 
                     <Landmark size={24} />}
                  </div>
                  <div className="text-center overflow-hidden w-full px-1">
                    <p className={`text-[10px] font-black uppercase truncate tracking-tight ${
                      transferType.endsWith('p1') ? 'text-blue-500' : 
                      transferType.endsWith('p2') ? 'text-rose-500' : 
                      'text-amber-600 dark:text-amber-500'
                    }`}>
                      {transferType.endsWith('p1') ? players[0].name : 
                       transferType.endsWith('p2') ? players[1].name : 
                       'The Bank'}
                    </p>
                  </div>
                </div>
          ...
          <div>
            <label className="block text-[10px] font-black text-slate-400 uppercase tracking-[0.25em] mb-3 text-center">Quick Select</label>
          ...
          <div className="relative pt-4">
            <label className="absolute top-0 left-1/2 -translate-x-1/2 text-[10px] font-black text-slate-400 uppercase tracking-[0.25em] bg-white dark:bg-slate-900 px-4">Amount</label>
